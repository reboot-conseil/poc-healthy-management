...
function getSpeakerIndex(speaker: string): number {
  ...
  return num % SPEAKER_PALETTES.length;
}
...
function UtteranceRow({ utterance, idx }: { utterance: Utterance; idx: number }) {
  const palette = SPEAKER_PALETTES[getSpeakerIndex(utterance.speaker)];
  const label = utterance.speaker.replace('SPEAKER_', '');
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2">
              ...
              <div>
                {report.content.utterances.map((utterance, idx) => (
                  <UtteranceRow key={utterance.id} utterance={utterance} idx={idx} />
                ))}
              </div>
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-20 space-y-6">
                ...
                {/* Speaker legend */}
                {report.content.utterances.length > 0 && (
                  <div>
                    <h3 className="font-display text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">
                      Participants
                    </h3>
                    <div className="space-y-2">
                      {[
                        ...new Set(report.content.utterances.map((u) => u.speaker)),
                      ].map((speaker) => {
                        const palette = SPEAKER_PALETTES[getSpeakerIndex(speaker)];
                        const count = report.content.utterances.filter(
                          (u) => u.speaker === speaker,
                        ).length;
                        return (
                          <div key={speaker} className="flex items-center gap-2.5">
                            <div className={cn('w-2 h-2 rounded-full shrink-0', palette.dot)} />
                            <span className="text-sm text-foreground font-mono">{speaker}</span>
